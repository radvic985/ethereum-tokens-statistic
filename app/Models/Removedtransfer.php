...
class Removedtransfer extends Model
{
    public $timestamps = false;
    protected $guarded = [];

    public static function getRemovedTransfers($id)
    {
        $transfers = Removedtransfer::all()->where('whale_id', $id)->sortByDesc('time_updated')->slice(0, 10);
        $data = [];
        if(!empty($transfers)) {
            $index = 0;
