...
class GetTopAddedRemoved extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'top_added_removed';
     ...
     * @var string
     */
    protected $description = 'Get top added and removed transfers';
    ...
    {
        \Log::info('TOP ADDED REM START - ' . \Carbon\Carbon::now());
                ...
                foreach ($output['operations'] as $item) {
                    ...
                    if (!empty($tokenOne)) {
                        ...
                        if ($whale->holder == $item['from']) {
                                ...
                                $removed[$index2]['quantity'] = $item['value'] * 1 / (10 ** (int)$item['tokenInfo']['symbol']);
                            ...
                            if ($whale->holder == $item['to']) {
                                ...
                                $added[$index1]['quantity'] = $item['value'] * 1 / (10 ** (int)$item['tokenInfo']['symbol']);
        ...
        \Log::info('TOP ADDED REM END NEWWWW- ' . \Carbon\Carbon::now());
            }
        }
    }
}
